...
void spi_write(int add,int ,int ,char *,int );
int spi_init(char *);
...
int spi_init(char filename[40])
    ...
    xfer[0].len = 32; /* Length of  command to write*/
    ...
    xfer[1].len = 33; /* Length of Data to read */
...
void spi_write(int add1,int add2,int nbytes,char value[10],int file)
    ...
    strncpy(buf, "Isto é um teste...", 16); 

    xfer[0].tx_buf = (unsigned long)buf;
    xfer[0].len = 16; /* Length of  command to write*/
...
int main(int argc, char *argv[]){
    ...
    int file=spi_init("//dev//spidev0.0"); //dev
